...
**Updated:** 2026-09-25  
**Status:** infrastructure feasibility passed  
...
The canonical v2 target in research/ATTRIBUTION_TARGET.md uses the standard
multiclass correct-class log-odds margin against all other classes.

That matches TRAK's classification model-output semantics. The technical smoke
therefore validates the same model-output family intended for development.

It is still infrastructure evidence only: it contains no Banking77 attribution
score and does not make head-restricted TRAK equivalent to full-model TRAK.
...
5. the canonical attribution target, sign, trajectory aggregation, and
   candidate aggregation remain unchanged.

The v2 attribution contract now satisfies item 5 prospectively.
