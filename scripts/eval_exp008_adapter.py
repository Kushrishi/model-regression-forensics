...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate one Experiment 008 explicit-policy adapter."
    )
    parser.add_argument(
        "--config",
    ...
    if config.experiment_id != "exp008":
        raise ValueError("evaluation requires Experiment 008 config")
    ...
    evaluate_lora_adapter_run(
        ...
        preparation_command="scripts/prepare_exp008.py",
    )
