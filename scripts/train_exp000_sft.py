...
from model_forensics.training import collate_sft_features, encode_sft_example, load_sft_examples
...
def main() -> None:
    ...
    train_path = prepared / "datasets" / f"{args.train_split}.jsonl"
    if not train_path.exists():
        raise FileNotFoundError(
            f"prepared training split not found: {train_path}; run scripts/prepare_exp000.py first"
        )
