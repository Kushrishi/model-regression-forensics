...
ORDER_NAMESPACE = "exp006-order-control-a"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare private Experiment 006 restoration and order-control datasets."
    )
...
def main() -> None:
    ...
    if config.experiment_id != "exp006":
        raise ValueError("certification preparation requires Experiment 006 config")
    ...
    data = build_exp006_data(config.seed, args.attempt_index)
    ...
    restorations = {
        candidate_id: build_exp006_restoration_train(
            candidate_id,
            seed=config.seed,
    ...
    summary = {
        ...
        "primary_restoration_count": len(EXP006_SHARD_IDS),
        "restored_records_per_candidate": EXP006_LABEL_CHANGES_PER_SHARD,
        "order_control_a": {
