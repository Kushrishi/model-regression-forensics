...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run blinded Experiment 002 RCA baselines.")
    parser.add_argument(
        "--diagnostic-manifest",
        default="artifacts/exp002/prepared/lineage/diagnostic.json",
    )
    parser.add_argument("--prepared", default="artifacts/exp002/prepared")
    parser.add_argument(
        "--baseline-generations",
        default="artifacts/exp002/runs/baseline/generations/target.jsonl",
    )
    parser.add_argument(
        "--candidate-generations",
        default="artifacts/exp002/runs/candidate/generations/target.jsonl",
    )
    parser.add_argument(
        "--output-root",
        default="experiments/002_entangled_distractors/diagnosis",
    )
...
def main() -> None:
    ...
    rankings = {
        "random": rank_candidates_random(manifest, seed=args.seed),
        "lexical_overlap": rank_candidates_lexical_overlap(
            manifest,
            prepared_root=args.prepared,
            regressions=regressions,
        ),
        "changed_lexical_overlap": rank_candidates_changed_lexical_overlap(
            manifest,
            prepared_root=args.prepared,
            regressions=regressions,
        ),
