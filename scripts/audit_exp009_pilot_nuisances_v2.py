...
def main() -> None:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("artifacts/exp009/pilot_nuisance_audit_v2"),
    )
    ...
    observed_pairs = tuple((pair.label_a, pair.label_b) for pair in selected)
    if observed_pairs != expected_pairs:
        raise AssertionError(
            "frozen v2 nuisance-pair selection drift: "
            f"expected={expected_pairs!r} observed={observed_pairs!r}"
        )
    ...
    aggregate: dict[str, object] = {
        ...
        "nuisance_rule_version": "v2",
        "frozen_pair_selection_match": True,
        "structural_match_for_diagnosis": False,
        "model_training_performed": False,
        "official_test_split_loaded": False,
    }
    _write_absent_or_identical(args.output_root / "summary.json", aggregate)

    print("===== EXP009 PILOT NUISANCE V2 PRE-TRAINING AUDIT =====")
    ...
    print("frozen_pair_selection_match=YES")
    print("structural_match_for_diagnosis=NO")
