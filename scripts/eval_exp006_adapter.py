...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate one Experiment 006 explicit-policy adapter."
    )
...
def main() -> None:
    ...
    if config.experiment_id != "exp006":
        raise ValueError("evaluation requires Experiment 006 config")
    ...
    evaluate_lora_adapter_run(
        ...
        preparation_command="scripts/prepare_exp006.py",
    )
